...
//cleanup function iterates through all data items and their corresponding write lists. For each wL, it checks if there is an older committed transaction that wrote the same data item. If it finds such a transaction, it removes the data item from the list of the current transaction.
void cleanup(int tx_id)
{
    pthread_mutex_lock(&cLock);
     for (int i = 0; i < m; i++) {
        // Step 1: Find the maximum timestamp for the data item that is less than or equal to the current transaction's timestamp
        int max_ts = -1;
        for (auto tx : v_Versions[i]) {
            if (tx.first <= tx_id && tx.first > max_ts) {
                max_ts = tx.first;
            }
        }
        // Step 2: Remove versions and transactions with timestamps greater than max_ts
        vector<int> txToRemove;
        for (auto tx : wL[i]) {
            if (tx == tx_id) continue; // do not remove the current transaction itself
            if (tx > max_ts) { // transaction has a timestamp greater than max_ts
                v_Versions[i].erase(tx);
                tx_log.erase(tx);
                txToRemove.push_back(tx);
            }
        }
        for (auto tx : txToRemove) {
            auto it = find(wL[i].begin(), wL[i].end(), tx);
            if (it != wL[i].end()) {
                wL[i].erase(it);
            }
        }
        // Step 3: Remove transactions that are older than the committed transaction
        vector<int> txToKeep;
        for (auto tx : wL[i]) {
            if (tx == tx_id || tx > max_ts) {
                txToKeep.push_back(tx);
            }
        }
        wL[i] = txToKeep;
    }
...
int read(int tx_id,int dx, int* ptLocal_Val)//Reads the data-item value into local variable for a tx
{
    ...
    if(pG == -1 || tx_id < maxRshd[dx]){//Transaction is aborted
        status[tx_id] = 1;
        cleanup(tx_id);
        pthread_mutex_unlock(&vLock);
        return 0;
